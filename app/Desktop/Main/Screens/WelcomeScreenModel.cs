...
sealed partial class WelcomeScreenModel : ObservableObject {
	...
	private sealed class SchemaUpgradeCallbacks(Window window) : ISchemaUpgradeCallbacks {
		public async Task<InitialDatabaseSettings?> GetInitialDatabaseSettings() {
			bool separateFileForDownloads = await Dialog.ShowYesNo(window, "Separate Database File for Downloads", "Do you want to use a separate database file for downloaded files?") == DialogResult.YesNo.Yes;
			bool downloadsAutoStart = await Dialog.ShowYesNo(window, "Automatic Downloads", "Do you want to automatically download files hosted on Discord? You can change this later in the Downloads tab.") == DialogResult.YesNo.Yes;
			return new InitialDatabaseSettings(separateFileForDownloads, downloadsAutoStart);
		}
